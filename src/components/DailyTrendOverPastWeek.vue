...
export default {
      ...
      xaxis: {
        ...
        labels: {
          formatter: (value, timestamp, index) => {
            return moment(new Date(timestamp)).format("ddd");
          }
        }
      },
      tooltip: {
  ...
  mounted() {
    axios
      .get(`${process.env.VUE_APP_API}/camera/${this.cameraId}/chart/past-week`)
      .then(response => {
    ...
    axios
      .get(`${process.env.VUE_APP_API}/camera/all/chart/past-week`)
      .then(response => {
